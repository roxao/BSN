		   ...
		   for ($j = 0; $j < count($query); $j++){
			...
			for ($i = 0; $i < count($uploaded); $i++) {
				...
				if ($this->input->post('upload') == "uploadstep6"){
					$this->user_model->update_document( $query[$j]->id_application, $query[$j]->id_application_file, $query[$j]->id_document_config, $uploaded['file_ext'], $username);

				} else if ($this->input->post('upload') == "uploadstep3"){
					$this->user_model->update_document( $query[$j]->id_application, $query[$j]->id_application_file, $query[$j]->id_document_config, $uploaded[$i]['file_ext'], $username);
