...
<div class="page" style="margin-top: 150px">
	<div class="clearfix">
		<ul id="section_progress" style="width: 100%">
			<li stepId="0" class="<?php $Status ; $Status= $this->session->flashdata('satu') == "COMPLETED" ? "COMPLETED": (  $this->session->flashdata('satu') == "PENDING"  ? 'PENDING' : ''); echo $Status ?>"><button>Pengajuan Surat Permohonan ke BSN</button></li>
			<li stepId="1" class="<?php $Status ; $Status= $this->session->flashdata('dua') == "COMPLETED" ? "PENDING": (  $this->session->flashdata('dua') == "COMPLETED"
					? 'COMPLETED' : ''
				   ); echo $Status ?>"><button>Hasil Verifikasi Status Permohonan</button></li>
