...
<form action="<?php echo base_url('admin_verifikasi_controller/VERIF_REVDOC_REQ_PROSES') ?>" method="post" accept-chaset="utf-8">
		<input type="hidden" name="id_application_status" value="">
		<input type="hidden" name="id_application" value="">
...
for (var i = 0; i < value.length; i++) {
	$('.attach_user_file').append('<div class="clearfix"><div>'+ (i+1) +'. '+ value[i].display_name +'</div><a href="<?php echo base_url();?>submit_iin/download?var1='+ value[i].path_id +'" class="btn_download float_right">Download</a></div>');
}
